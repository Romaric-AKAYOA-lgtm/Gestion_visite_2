...
class ClProgrammeVisiteForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if hdbt and hhf and hhf < hdbt:
            self.add_error('hhf', "L'heure de fin doit être supérieure ou égale à l'heure de début.")

        # Validation : Motif obligatoire si statut annulé
        if statut == 'annulé':
            if not motif:
